...
def insert_to_db(info):
    ...
    db = client['vacancy']
    cplusplus = db.cplusplus

    db.cplusplus.insert_many(info)
    for item in cplusplus.find({}):
        pprint(item)
...
def db_find():
    ...
    salary = input('Enter currency: ')

    for vacancy in db.cplusplus.find({'$or': [{'salary_min': {'$gt': salary}}, {'salary_max': {'$gt': salary}}]}):
        pprint(vacancy)

def db_updata(cplusplus, info):
    if not cplusplus.find_one({'_id': info['_id']}):
        cplusplus.insert_one(info)
